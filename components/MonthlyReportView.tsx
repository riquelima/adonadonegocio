    ...
    <div className="bg-white shadow-lg rounded-xl transition-shadow hover:shadow-xl">
      ...
      >
        <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-4">
          <div>
            <p className="font-bold text-lg sm:text-xl text-[#0D2B3E]">{summary.monthYear}</p>
...
const MonthlyReportView: React.FC<MonthlyReportViewProps> = ({ summaries, onView, onEdit, onDelete, isBusy }) => {
  if (summaries.length === 0) {
    return (
       <div className="max-w-4xl mx-auto">
        <header className="mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#0D2B3E]">Resumo Mensal</h2>
        </header>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="space-y-6">
        {summaries.map(summary => (
          <MonthlyCard key={summary.monthYear} summary={summary} onView={onView} onEdit={onEdit} onDelete={onDelete} isBusy={isBusy} />
        ))}
      </div>
    </div>
  );
